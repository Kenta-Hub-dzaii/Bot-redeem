...
def generate_random_key():
    """Tạo key ngẫu nhiên dạng xxxxx-xxxxx-xxxxx-xxxxx-xxxxx"""
    parts = []
    for _ in range(5):
        ...
        if user_data.get('key') == key_to_verify:
            ...
            if user_data.get('hwid') == hwid:
                ...
                if last_reset:
                    ...
                    if get_current_time() - last_reset_time < timedelta(minutes=1):
...
async def redeem_key_cmd(interaction: discord.Interaction, key: str):
    ...
    if 'error' in result:
        ...
        await interaction.response.send_message(
            f"✅ Redeem thành công! Key mới của bạn: `{result['new_key']}`", 
            ephemeral=True
        )
